...
namespace ProyekACS
{
    public partial class NewSalesOrderAdmin : Form
    {
       
        public NewSalesOrderAdmin()
        {
            InitializeComponent();
        }
        ...
        ArrayList listKaryawan = new ArrayList();
        private void NewSalesOrderAdmin_Load(object sender, EventArgs e)
        {
            ...
            queryString = "select id,nama from karyawan where fk_jabatan >0";
            command = new OracleCommand(queryString, conn);
            using (OracleDataReader reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    cmbAssignTo.Items.Add(reader.GetString(1));
                    listKaryawan.Add(reader.GetInt32(0));
                }
            ...
            String queryStr = "SELECT * FROM JENIS_SABUN ";
            //query.CommandText = queryStr;
            //adapter.Fill(ds, "JENIS_SABUN");

            //queryStr = "SELECT * FROM SATUAN_PENJUALAN_NETT ";
            //query.CommandText = queryStr;
            //adapter.Fill(ds, "SATUAN_PENJUALAN_NETT");

            //queryStr = "SELECT * FROM SABUN";
            //query.CommandText = queryStr;
            //adapter.Fill(ds, "SABUN");

            //queryStr = "SELECT * FROM KARYAWAN  ";
            //query.CommandText = queryStr;
            //adapter.Fill(ds, "KARYAWAN ");

            //queryStr = "SELECT * FROM JABATAN   ";
            //query.CommandText = queryStr;
            //adapter.Fill(ds, "JABATAN ");

            //queryStr = "SELECT * FROM PELANGGAN    ";
            //query.CommandText = queryStr;
        ...
        private void btnAddRow_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show(Convert.ToString(listStok[cmbSatuan.SelectedIndex]));
            MessageBox.Show(Convert.ToString(kali));
            MessageBox.Show(Convert.ToString(Qty.Value));
            ...
            {
                total += Convert.ToInt32(textAmount.Text);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd = new OracleCommand($"insert into h_trans values('{textNomorSO.Text}'," +
                    ...
                    $" {total}, " +
                    $"{listKaryawan[cmbAssignTo.SelectedIndex]}, " +
